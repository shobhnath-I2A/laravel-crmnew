                                ...
                                <div class="card-body">
                                    <h4 class="card-title cardtitle">TransferMaster
                                        ...
                                        <div class="float-right">
                                            ...
                                            <button type="button" class="btn btn-secondary btn-lg waves-effect waves-light"
                                                onclick="openSidebar('Add Transfer','{{ route('transfers.create') }}')"
                                                data-backdrop="static">Add Transfer</button>
                                        </div>
                                    </h4>
                                    <div class="table-responsive">
                                        <table class="table table-hover mb-0">
                                            ...
                                            <tbody>
                                                @foreach ($transferMaster as $transfer)
                                                    <tr>
                                                        <td width="35%" style="cursor:pointer;" onclick="openPopup('Update Price', '{{ route('transfer-rate-list.show', $transfer->id) }}')">
                                                        ...
                                                        <td width="15%">{{ $transfer->destinationCity->name ?? '' }}</td>
                                                        <td width="1%" align="left">
                                                            <div align="center">
                                                                <a class="dropdown-item"
                                                                    style="cursor:pointer; font-size:12px; text-decoration:underline;"
                                                                    onclick="openPopup('Update Price', '{{ route('transfer-rate-list.show', $transfer->id) }}')"
                                                                    >
                                                                    Update</a>
                                                            </div>
                                                        </td>
                                                        <td width="1%" align="left">
                                                           <span class="badge {{ $transfer->status == 1 ? 'badge-success' : 'badge-danger' }}"> {{ $transfer->status == 1 ? 'Active' : 'Inactive' }} </span>
                                                        </td>
                                                        ...
                                                        <td width="12%" align="left">{{ $transfer->created_at }}</td>
                                                        <td width="1%">
                                                            <a class="dropdown-item neweditpan" onclick="openSidebar('Edit Transfer', '{{ route('transfers.edit', $transfer->id ) }}')">
                                    ...
                                    <div class="mt-3 pageingouter">
                                        <div style="float: left; font-size: 13px; padding: 7px 11px; border: 1px solid #ededed; background-color: #fff; color: #000;">
                                            Total Records: <strong>{{ $transferCount }}</strong></div>
                                        <div class="d-flex justify-content-end">
                                            {{ $transferMaster->links() }}
                                        </div>
                                    </div>
                                </div>
                            </div>
