...
<div class="wrapper" style="margin-top: 0px; padding:15px;">
    <form class="custom-validation ajax-form"
        action="{{ isset($leadSource) ? route('lead-source.update', $leadSource->id) : route('lead-source.store') }}"
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="status" class="form-control reqfield @error('status') is-invalid @enderror">
                            ...
                            <option value="1" {{ old('status', $leadSource->status ?? '') == 1 ? 'selected' : '' }}>
                                Active
                            </option>
                            <option value="0" {{ old('status', $leadSource->status ?? '') == 0 ? 'selected' : '' }}>
                                Inactive
                            </option>
                        </select>
                        @error('status')
